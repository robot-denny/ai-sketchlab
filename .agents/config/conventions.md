...
- `<TreatWarningsAsErrors>true</TreatWarningsAsErrors>` + `<Nullable>enable</Nullable>` on all
  four C# projects (`UmbracoProject`, `UmbracoProject.Features`, `HelloWorld`, `UmbracoProject.Tests`)
  — any new warning fails the build. Relax only via surgical per-code
