...
> Migration in progress: the repo also has legacy `_specs/`, `_plans/`, and `_audits/` holding
> pre-cantrip history. **New** increments use `_work/`; migrating the legacy dirs is deferred.
> Do not write new specs/plans into `_specs/` or `_plans/`.
