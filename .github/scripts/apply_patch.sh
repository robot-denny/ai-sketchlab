...
if git apply "$patchFile" --ignore-space-change --ignore-whitespace --check; then
    ...
    if [[ "$pipelineVendor" == "GITHUB" ]]; then
        # Fixed locally: upstream used `git add *`, which the shell expands and
        # skips dotfiles (.gitattributes, .editorconfig, etc.). Use --all so
        # Cloud-side dotfile changes are committed back. The AZUREDEVOPS branch
        # below was already correct; this aligns GITHUB with it.
        git add --all
