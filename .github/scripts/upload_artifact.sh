...
function call_api {
  ...
  if [[ 10#$responseCode -eq 200 ]]; then
    ...
    # Fixed locally: upstream used `cat "$content"`, but $content is the JSON
    # response body (a string), not a file path. cat would silently fail.
    echo "$content"
  ...
  # Fixed locally: upstream used `cat "$errorResponse"`, but errorResponse is
  # the response body (a string), not a file. cat would silently fail.
