...
`cloud-deployment` reports "finished" before the deployed Dev app is actually *serving* search — `Umbraco.AI.Search` can come up **cold** (its vector data survives the deploy, but the searcher isn't hydrated yet, so the vector query returns near-empty). Because `AI.Search` hooks the publish pipeline to embed each document, `POST /document` also misbehaves while cold — and `playwright-against-dev` creates its fixtures via `POST /document`, so one cold subsystem used to cascade into ~9 misleading test failures needing a hand Portal restart + `gh run rerun --failed`.

The `playwright-against-dev` job runs [`.github/scripts/wait_for_search_warm.sh`](.github/scripts/wait_for_search_warm.sh) (env `URL` only) right after checkout, replacing the old thin "Sanity check Dev is up" home-page curl and running *before* the Playwright step. It **polls `GET $URL/search?q=article`** for the `article-grid-card` serving marker (every 10s); as soon as it serves the gate opens and Playwright proceeds. If still cold at the budget (`TOTAL_BUDGET`, default **180s**) it **fails fast with one actionable diagnostic and does NOT launch Playwright** — a single legible failure instead of the ~7-9-test cascade. The budget is deliberately short: Dev search self-warms but takes ~20-30 min (see next paragraph), far longer than any CI-reasonable wait, so failing fast + wait-then-rerun beats parking a runner.

**This gate detects and fails fast; it does NOT self-heal — and the remedy is WAIT, not restart (corrected 2026-07-02).** Dev search **self-warms in ~20-30 min** after a deploy; neither a rebuild nor a restart is needed. Two disproven "fixes":
- *Rebuild:* an earlier gate fired a `UmbAI_Search` Management-API rebuild; run `28539245107` showed it repopulates the index (→179/Healthy) but doesn't make `/search` serve any sooner (and correlated with `"index was locked"` errors, likely beta.9 `Provider.Examine`). Removed.
- *Restart:* earlier notes claimed "only an app restart rehydrates the searcher." **Misdiagnosis** — restarts only *seemed* to work because ~20-30 min had passed. Run `28624753905` (2026-07-02) proved it: left **untouched**, `/search?q=article` climbed 0 → 10 over ~20-30 min, and a `--failed` rerun once warm passed the gate + Playwright green **with no restart**.

So when the gate fails: **wait ~20-30 min** (poll `curl -s $URL/search?q=article | grep -c article-grid-card` until > 0), then `gh run rerun <id> --failed`. Do **not** restart Dev. Measure true warm-up time with [`.github/scripts/measure_search_warmup.sh`](.github/scripts/measure_search_warmup.sh) to inform any budget change. Eliminating the manual rerun is tracked as follow-up (raise budget to measured warm-up, or make fixture creation cold-resilient). **v18-upgrade revisit:** re-verify warm-up timing + this readiness marker on the new search stack. Full playbook: [CI Failure Recipes → cold AI.Search 500 cascade](docs/ci-failure-recipes.md).
