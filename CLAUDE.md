...
So when the gate fails: **restart Dev from the Portal**, confirm `curl -s $URL/search?q=article | grep -c article-grid-card` returns `> 0`, then `gh run rerun <id> --failed`. Eliminating the per-deploy toil is tracked as follow-up: re-point the probe at a long/semantic query (the Healthy path), make fixture creation resilient to a corrupt keyword index, or move to a stable `Provider.Examine`. **v18-upgrade revisit:** a stable `Provider.Examine` should end the corruption (pinned at beta.9 only because no stable exists). Full playbook: [CI Failure Recipes → cold AI.Search 500 cascade](docs/ci-failure-recipes.md).
