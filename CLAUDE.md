...
The `playwright-against-dev` job runs [`.github/scripts/wait_for_search_warm.sh`](.github/scripts/wait_for_search_warm.sh) (env `URL` only) right after checkout, replacing the old thin "Sanity check Dev is up" home-page curl and running *before* the Playwright step. It **polls `GET $URL/search?q=article`** for the `article-grid-card` serving marker (every 10s); as soon as it serves the gate opens and Playwright proceeds. If still cold at the budget (`TOTAL_BUDGET`, default **180s** — a genuinely cold deploy never self-warms, so a long wait is pointless) it **fails fast with one actionable diagnostic and does NOT launch Playwright** — a single legible failure instead of the ~9-test cascade.

**This gate detects and fails fast; it does NOT self-heal.** An earlier version fired a `UmbAI_Search` Management-API rebuild as a CI substitute for a restart (CI can't restart — the Cloud CI/CD Flow API has no restart endpoint). A real cold master run (2026-07-01, run `28539245107`) **disproved it**: the rebuild repopulated the index (→179/Healthy) but did **not** rehydrate the running searcher (`/search` stayed cold), and it correlated with `"index was locked"` errors (likely the beta.9 `Provider.Examine` fragility — see *Pinned betas*). Only an app **restart** (Cloud Portal) rehydrates the searcher. So when the gate fails: restart Dev from the Portal, confirm `/search` serves, then `gh run rerun <id> --failed`. **v18-upgrade revisit:** the search stack is still beta — re-verify cold-serving behavior, any real warm/restart lever, and this gate's readiness marker on the new version. Full playbook: [CI Failure Recipes → cold AI.Search 500 cascade](docs/ci-failure-recipes.md).
