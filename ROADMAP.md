...
- **fix-e2e-dev-only-failures** — the Playwright-against-Dev gate has **5 distinct failures that pass locally and fail only on Dev**, so they're Dev environment/content gaps, not test bugs (verified 2026-05-27 by running each against a local site; local and Dev render byte-identically). Resolving them takes the gate green and also unblocks ~27 serial-sibling tests currently reported "did not run". (1) **imageGenerator** — [dashboard.spec.ts:141](tests/e2e/imageGenerator/dashboard.spec.ts#L141) "generate endpoint succeeds end-to-end": set/verify Dev's `OPENAI__APIKEY` in Cloud Secrets Management + image-gen config. (2) **guides-cli** (3 tests in [guides-cli.spec.ts](tests/e2e/guides-cli.spec.ts)): the guide-generator CLI runs against Dev's Management API with its own OAuth client — check the Dev client id/secret/scopes. (3) **imageCarousel** — [imageCarousel.spec.ts:742](tests/e2e/blocks/imageCarousel.spec.ts#L742) "first image has a non-empty alt attribute": the carousel slide media on Dev has no alt text (content fix). Surfaced 2026-05-27 during the E2E-rot repair (the systemic token-fixture + stale-selector/v2-token failures were fixed in that pass; these Dev-only items are the residue) → no spec yet
