...
**Source spec**: `_specs/shipped/fix-e2e-dev-only-failures.md`
**Last verified**: 2026-05-29 — Gate 2 on master post-merge: 249 passed / 26 failed / 19 skipped of 294 total (vs baseline 218 passed / 31 failed / ~27 not-run). All 4 originally-targeted tests pass + 1 deliberately skipped (dashboard.spec.ts:141 on Cloud URLs). The 26 remaining failures were filed as the parking-lot ROADMAP entry `triage-gate2-unmasked-failures` — **resolved 2026-06-04**: they were not sibling functional specs (that was an unverified guess) but the 26 un-baselined screenshot specs from `arch-safety-net`; see the 2026-06-04 Revision Note below.
...
- [x] Fix 4 of 5 known Dev-only Playwright failures + defer the 5th + capture the diagnostic playbook in CLAUDE.md — shipped 2026-05-29 (spec: `_specs/shipped/fix-e2e-dev-only-failures.md`, plan: `_plans/shipped/fix-e2e-dev-only-failures.md`)
...
- [x] Parking lot — triage the 26 newly-surfaced Gate 2 failures (`triage-gate2-unmasked-failures`) — **resolved 2026-06-04**: they were the un-baselined `arch-safety-net` screenshot specs, not sibling functional specs. Fixed by repairing `update-snapshots.yml` (commits `db1df8f`, `5d4cdb1`) and committing baselines (`b2949db`). See 2026-06-04 Revision Note.
...
- 2026-06-04 (triage-resolved): **The 2026-05-29 characterisation of the 26 failures was wrong.** `triage-gate2-unmasked-failures` had clustered them onto functional specs (articleListGridView ~12 / ellaBlockAttribution ~8 / alertBanner.spec ~8 / articleCardMetaDescription ~5 / authors ~2) — an unverified guess (its own text deferred "click through for the precise per-test failure list"; the cluster counts summed to ~35, not 26; and articleListGridView is `describe.skip`-ed, i.e. *skipped* not *failed*). Pulling the precise failed lists from both the 2026-05-29 (sha 0dcca37) and 2026-06-02 (sha 139ba7e) master Playwright jobs shows the 26 were exactly the 20 block + 6 page **screenshot specs** from `arch-safety-net` — none of the named functional specs appear. Root cause: those specs shipped without their Linux baselines ever being committed, so `toHaveScreenshot` failed on every run; and `update-snapshots.yml` (the only baseline-generation path) was broken two ways (Playwright 1.56 `--update-snapshots` mode requirement, and a missing `UMBRACO_BASE_URL` that sent `guides.spec.ts`' CLI to localhost). Fixed in `db1df8f` + `5d4cdb1`; baselines committed `b2949db`. ROADMAP `triage-gate2-unmasked-failures` removed from Next; resolution recorded under Recently shipped as `fix-screenshot-baselines`.
