...
Scope: the toggle lives on the standard `Visibility Controls` composition (composed by Content, Documentation, Article, etc.) — the page types that actually render the section-nav sidebar (via `content.cshtml` / `documentation.cshtml`). It is **not** on the separate `Guide Visibility Controls` composition used by Guides / How-To Guide pages: a 2026-07-08 attempt to mirror it there was reverted (guide pages render no section nav of their own, so the toggle was latent, and the addition got stuck in Cloud schema propagation). Revisit if/when guides are reworked — see ROADMAP `guides-rework`.
