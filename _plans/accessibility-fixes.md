...
- **Focus-ring style = `2px solid` + `2px` offset, colour consistently accent-red on light surfaces.** *(Colour decided 2026-06-04 during Step 4 review; supersedes the earlier "leave colour contextual" deferral of Step-2 Nit #5.)* The ring colour was previously a mix — `--accent-primary` red on content links, `currentColor` (dark) on nav/footer/search chrome, `--text-on-dark` on dark surfaces — and `styles.css`'s brown `--site-link-color` ring was dead (overridden by typography's red). Standardize: **all light-surface focus rings use `var(--accent-primary)` (red, ≥3:1 on every light surface incl. alert tints and `--surface-secondary`)** — base `a:focus-visible` in both `styles.css` + `typography.css`, the `#mainNav/.section-nav/.pagination` rule, and the `.site-head .site-nav`/`.search-ico`/`.foot .col a` chrome rules. **Dark/accent surfaces keep `--text-on-dark`** (`header.masthead`, `.on-dark`, `.section-row--bg-accent`, legacy `.site-footer .footer-nav`) — a red ring would vanish on the red accent surface. The `.search-ico` is a `<button>` so it still needs its own rule (base `a:focus-visible` won't match it).
