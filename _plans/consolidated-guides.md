...
> **⏸ STATUS: PAUSED / BLOCKED (2026-07-23).** Blocked on a newly-discovered prerequisite: the Experiments-era `exp-*` blocks (`featureCard`, `statCallout`, `pullQuoteBlock`, `commandBadge`, `timelineRow`, `showcaseHero`, `embeddedSketch`) render **unstyled** on the guide pages because their functional CSS **and the Block Grid layout engine itself** are scoped to `main.experiments` / `.exp-pillar` inside `wwwroot/assets/css/experiments.css` (a block-CSS-seam violation — see `docs/block-css-seam.md`). You cannot art-direct or baseline blocks that don't render. **Prerequisite increment: "Block Grid CSS portability"** (de-scope the grid engine off `main.experiments` + decouple the `exp-*` block CSS from `experiments.css`) — see ROADMAP. **Done so far (committed):** the layout/type half — the grid body is constrained to the `.styleguide` container (no longer full-bleed) and the section heading hierarchy is fixed (no more duplicate sibling h2s). **Still blocked:** block-level art direction, `showcaseHero`/`pillarSection` scoping decision for the Component Guide, and the guide-page screenshot baselines. **Steps 8–9 are gated behind completing this step.**
>
