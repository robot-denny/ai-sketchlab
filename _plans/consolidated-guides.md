...
> **✅ STATUS: DONE (2026-07-23).** The prerequisite — the **`block-grid-css-portability`** increment (global `blocks.css` Block Grid engine + decoupled `exp-*` block CSS) — shipped and merged to master; `consolidated-guides` merged master back in, so the guide pages now render every block correctly (verified by screenshot: featureCard, statCallout, pullQuote, commandBadge, timeline, embeddedSketch all styled). **Completed:** (a) width — grid body wrapped in the `.styleguide` 780px container (no longer full-bleed); (b) section separation via the existing `.styleguide__section-anchor` rhythm; (c) heading hierarchy — section title `h2` dominant, block headings stepped down (no duplicate sibling h2s). **`showcaseHero` removed from the Component Guide** (page-composition, renders only on Experiments; its removal also dropped a stray mid-page `<h1>`, so the heading outline is clean — the a11y heading-order check passes). `pillarSection` is grid-only and was never in the Guide Body palette. **Remaining (not gating Step 8/9):** guide-page + retargeted-block screenshot **baselines** must be regenerated on Linux CI (`update-snapshots.yml`) once the content reaches Dev — the definitive pixel gate.
>
