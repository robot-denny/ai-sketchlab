...
**What the gate does (detect + fail-fast)** — [`.github/scripts/wait_for_search_warm.sh`](../.github/scripts/wait_for_search_warm.sh), before the Playwright step, polls `GET $URL/search?q=article` for the `article-grid-card` marker. Serves → gate opens. Still cold at the budget → **fails fast, does NOT launch Playwright**, so you get one clear failure instead of the ~7-9-test cascade. (The probe is a SHORT query, so it tracks the fragile keyword index — see follow-up on re-pointing it at the semantic path.)
...
**Why this recurs / durable fix** — every master deploy re-corrupts the Examine keyword index (beta.9 fragility), so restart-then-rerun is the current remedy. To stop the per-deploy toil: (a) re-point the gate probe at a LONG/semantic query so CI reflects the Healthy path, (b) make `/search` + fixture creation resilient to a corrupt keyword index, or (c) move to a **stable `Provider.Examine`** at the v18 upgrade (it's pinned at beta.9 only because no stable exists). Tracked as follow-up. `.github/scripts/measure_search_warmup.sh` remains useful to confirm a deploy is/ isn't recovering.
