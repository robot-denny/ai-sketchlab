...
**This is a cold subsystem, NOT an empty index.** The correct move is to **restart the Dev environment from the Cloud Portal** — this rehydrates the running searcher (search serves within ~15s afterwards). Do **not** rebuild the index: the data is already fine (`documentCount` stays ~179/Healthy), and a rebuild does **not** fix the serving state (see the disproof below).

**⚠️ A `UmbAI_Search` rebuild does NOT fix this — empirically disproven.** An earlier version of the gate tried to self-heal by firing a Management API rebuild (`PUT {URL}/umbraco/search/api/v1/rebuild?indexAlias=UmbAI_Search`) as the CI-reachable substitute for a restart (CI can't restart — the Cloud CI/CD Flow API has no restart endpoint). A **real cold master run (2026-07-01, run `28539245107`)** proved it wrong: the gate fired the rebuild, the index went to **179/Healthy**, but `/search` stayed cold for the full budget and remained cold long after. A rebuild repopulates the **index (writer side)** but does **not** rehydrate the running app's **searcher (reader/query side)** — only an app restart does. Worse, firing the rebuild correlated with `"The index was locked and could not be unlocked"` errors in Dev's log (likely the beta.9 `Provider.Examine` fragility — see *Pinned betas* in CLAUDE.md). The rebuild was therefore **removed**; the gate is now detect-and-fail-fast only.

**What the gate does now (detect + fail-fast, NOT self-heal)** — [`.github/scripts/wait_for_search_warm.sh`](../.github/scripts/wait_for_search_warm.sh), wired into `playwright-against-dev` (it replaced the old "Sanity check Dev is up" home-page curl, and runs *before* the Playwright step), polls `GET $URL/search?q=article` for the `article-grid-card` serving marker. If it serves → gate opens, Playwright proceeds. If still cold at the budget (default `TOTAL_BUDGET=180s` — a genuinely cold deploy never self-warms, so a long wait is pointless) → it **fails fast with one actionable diagnostic and does NOT launch Playwright**, so you get a single clear failure instead of the ~9-test cascade.

**When the gate fails** (`did not come up serving within …`): restart Dev from the Cloud Portal, confirm `curl "$DEV_URL/search?q=article"` returns results, then `gh run rerun <run-id> --failed` (this re-runs only the Playwright job; the deploy jobs already succeeded, so it won't re-deploy and re-cold the app).

**Verification** — the fail-fast half was proven on cold run `28539245107` (gate detected cold, failed fast, Playwright cleanly skipped — no cascade); the happy path was proven on the same run's `--failed` rerun after a manual Dev restart (gate saw search serving, passed, Playwright green).

**Why this recurs** — every master merge triggers a fresh Dev deploy, and whether the searcher comes up warm or cold is timing-dependent (not controllable from CI). The gate makes the *failure* deterministic and legible; it does not prevent the cold state. Repeated gate failures = a real Dev search-health problem, not noise.
