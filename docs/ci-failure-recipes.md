...
**Root cause** — `Umbraco.AI.Search` comes up **cold** after a Gate 2 deploy to Dev. `cloud-deployment` reports "finished" before the deployed app is actually *serving* search: the vector data survives the deploy (`documentCount` stays ~179), but the search indexes (Examine keyword + AI vector) **hydrate gradually over ~20-30 min** and don't serve until then. Because `AI.Search` hooks the publish pipeline to embed each document, every `POST /document` throws `500` while the embedding path is cold — and Playwright creates its fixtures via `POST /document`, so one cold subsystem cascades into ~7-9 misleading test failures. **The searcher is not broken — it just needs time (see the corrected remedy below).**
...
**This is a cold subsystem that SELF-WARMS — the correct move is to WAIT, not to restart.** Dev's search indexes hydrate gradually after a deploy and start serving in ~20-30 min with **no intervention**. Do **not** restart Dev and do **not** rebuild the index — both were tried and neither speeds it up (disproofs below).

**⚠️ A `UmbAI_Search` rebuild does NOT fix this — empirically disproven.** An earlier gate version fired a Management API rebuild (`PUT {URL}/umbraco/search/api/v1/rebuild?indexAlias=UmbAI_Search`) as a CI-reachable self-heal. A **real cold master run (2026-07-01, run `28539245107`)** proved it wrong: the index went to **179/Healthy** but `/search` stayed cold. A rebuild repopulates the **index (writer side)** but doesn't make the query path serve any sooner, and firing it correlated with `"The index was locked and could not be unlocked"` errors (likely beta.9 `Provider.Examine` fragility — see *Pinned betas* in CLAUDE.md). The rebuild was **removed**.

**⚠️ A Portal RESTART is ALSO not required — CORRECTED 2026-07-02.** Earlier revisions of this runbook claimed "only an app restart rehydrates the searcher." That was a **misdiagnosis**: restarts *appeared* to fix it only because ~20-30 min had already passed by the time someone restarted and reran. A real cold deploy (run `28624753905`) was left **completely untouched** and `GET /search?q=article` climbed **0 → 10 results over ~20-30 min on its own**; re-running the failed job once warm passed the gate and Playwright went green **with no restart and no rebuild**. So `/search` self-warms given time — the default `TOTAL_BUDGET=180s` is simply ~10× shorter than the real warm-up.

**What the gate does now (detect + fail-fast)** — [`.github/scripts/wait_for_search_warm.sh`](../.github/scripts/wait_for_search_warm.sh), wired into `playwright-against-dev` (before the Playwright step), polls `GET $URL/search?q=article` for the `article-grid-card` serving marker. If it serves → gate opens, Playwright proceeds. If still cold at the budget → it **fails fast with one diagnostic and does NOT launch Playwright**, so you get a single clear failure instead of the ~7-9-test cascade. The budget stays short (180s) **on purpose**: a genuinely cold deploy won't warm within any CI-reasonable window, so failing fast + wait-then-rerun is cheaper than parking a runner for ~30 min.

**When the gate fails** (`did not come up serving within …`): **wait ~20-30 min** for warm-up — poll `curl -s "$DEV_URL/search?q=article" | grep -c article-grid-card` until it returns `> 0` — then `gh run rerun <run-id> --failed` (re-runs only the Playwright job; the deploy jobs already succeeded, so it won't re-deploy and re-cold the app). **Do NOT restart Dev** — it doesn't help and wastes time. Measure the real warm-up time with `.github/scripts/measure_search_warmup.sh` (see below) if you want to tune the budget.

**Verification** — fail-fast proven on cold run `28539245107`; the **self-warm + no-restart** happy path proven on run `28624753905` (2026-07-02): cold at the 180s gate, left untouched, `/search` self-warmed to 10 results in ~20-30 min, then `--failed` rerun passed gate + Playwright green with no restart or rebuild.
